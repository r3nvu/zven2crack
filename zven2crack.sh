  ...
  while true; do
    ...
    case "$choice" in
      1)
        ...
        read -rp "Path to dictionary (or leave empty to generate later): " dict
        if [ -z "$dict" ]; then
          echo "No dictionary specified."; continue
        fi
        run_archive "$archive" "$dict"
        ;;
      2)
        read -rp "Output filename (optional, leave empty for timestamped in dictionary/): " out
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --charset)
        CHARSET="$2"; shift 2;;
      --help|-h)
        echo "Usage: $0 gen [--output path] [--min N] [--max M] [--charset CHARS]"; return 0;;
